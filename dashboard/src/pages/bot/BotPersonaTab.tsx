...
export default function BotPersonaTab() {
  ...
  const handleCreateCustom = async () => {
    ...
    try {
      ...
      const newPersona: Persona = {
        ...
        description: newDescription.trim() || t('botPersona.customPersonaDescription', 'Custom persona: {{name}}', { name: newName.trim() }),
  ...
  const handleGenerateFromWebsite = async () => {
    ...
    const taskId = addTask(botId, 'persona', t('botPersona.generatingFromWebsite', 'Generating from website...'))
    ...
    try {
      ...
      const personaName = data.business_name
        ? t('botPersona.websiteAssistantNamed', '{{name}} Assistant', { name: data.business_name })
        : t('botPersona.websiteAssistant', 'Website Assistant')
      const newPersona: Persona = {
        id: customId,
        name: personaName,
        emoji: CUSTOM_PERSONA_EMOJI,
        description: t('botPersona.autoGeneratedDescription', 'Auto-generated from {{source}} trained content.', {
          source: data.business_name || t('botPersona.website', 'website'),
        }),
      ...
      updateTask(taskId, 'done', t('botPersona.generatedSuccess', 'Persona generated!'))
    } catch (error) {
      updateTask(taskId, 'error', t('botPersona.generatedFailed', 'Failed to generate persona'))
  ...
  const categoryLabel = (category: string) => {
    if (category === ALL_TAB) return t('botPersona.categoryAll', 'All')
    if (category === CUSTOM_CATEGORY) return t('botPersona.categoryCustom', 'Custom')
    return category
  }

  if (!botId || !selectedBot) {
    return <div className="empty-panel">{t('botPersona.selectBot', 'Select a bot to configure personas.')}</div>
  }

  return (
    <AnimatedPage>
      <SectionHeader
        eyebrow={t('botPersona.eyebrow', 'Personality')}
        title={t('botPersona.title', 'Choose a persona for your AI')}
        subtitle={t('botPersona.subtitle', 'Select a personality that defines how your agent communicates. This shapes the tone, style, and character of every response.')}
      />
      ...
      <div style={{ display: 'flex', gap: '0.75rem', marginBottom: '1rem', alignItems: 'stretch' }}>
        <div className="persona-search-bar" style={{ flex: 1, marginBottom: 0 }}>
          ...
          <input
            ...
            placeholder={t('botPersona.searchPlaceholder', 'Search personas...')}
            value={searchQuery}
        ...
        >
          <Globe size={16} />
          {generatingPrompt ? t('botPersona.generating', 'Generating...') : t('botPersona.generateFromWebsite', 'Generate from website')}
        </UiButton>
        <UiButton
          variant="primary"
        ...
        >
          <Plus size={16} />
          {t('botPersona.createCustom', 'Create Custom')}
        </UiButton>
      </div>
      ...
      {/* Persona grid */}
      {loading ? (
        <div className="persona-loading">{t('botPersona.loadingPersonas', 'Loading personas...')}</div>
      ) : (
              ...
              >
                <div className="persona-card-header">
                  ...
                  <div className="persona-card-meta">
                    ...
                    <span className="persona-card-category">
                      ...
                      {isCustom && <span className="persona-custom-badge">{t('botPersona.categoryCustom', 'Custom')}</span>}
                    </span>
                  </div>
                  {isSelected && (
                ...
                <div className="persona-card-footer">
                  <div className="persona-card-footer-actions">
                    ...
                    >
                      {t('botPersona.previewPrompt', 'Preview prompt')}
                    </button>
                    <button
                      type="button"
                      ...
                      disabled={saving}
                    >
                      {saving && applyingPersonaId === p.id
                        ? t('botPersona.applying', 'Applying...')
                        : isSaved
                          ? t('botPersona.applied', 'Applied')
                          : t('botPersona.applyPersona', 'Apply persona')}
                    </button>
                  </div>
                  {isCustom && (
                    <button
                      ...
                      title={t('botPersona.deleteCustomPersona', 'Delete custom persona')}
                    >
                      <Trash2 size={13} />
                    </button>
                  )}
      ...
      {filteredPersonas.length === 0 && !loading && (
        <div className="persona-empty">
          {t('botPersona.noMatch', 'No personas match your search. Try a different term or category.')}
        </div>
      )}
          ...
          <div className="persona-modal" onClick={(e) => e.stopPropagation()}>
            <div className="persona-preview-header">
              ...
              <div>
                ...
                <span className="persona-preview-cat">{categoryLabel(previewPersona.category)}</span>
              </div>
            ...
            <div className="persona-preview-prompt-label">{t('botPersona.systemPrompt', 'System Prompt')}</div>
            ...
            <div className="persona-preview-actions">
              ...
              >
                {t('botPersona.useThisPersona', 'Use this persona')}
              </UiButton>
            </div>
          </div>
        </div>
      ...
      {showCreateModal && (
        <div className="persona-modal-overlay" onClick={() => setShowCreateModal(false)}>
          <div className="persona-modal persona-modal--create" onClick={(e) => e.stopPropagation()}>
            <div className="persona-preview-header">
              <div>
                <h4 className="persona-preview-name">{t('botPersona.createCustomPersona', 'Create Custom Persona')}</h4>
                <span className="persona-preview-cat">{t('botPersona.createCustomSubtitle', 'Design your own personality')}</span>
              </div>
            ...
            <div className="persona-create-form">
              <GlassField label={t('botPersona.nameLabel', 'Name')} className="persona-create-field" style={{ maxWidth: '100%' }}>
                <input
                  type="text"
                  placeholder={t('botPersona.namePlaceholder', 'e.g. Friendly Sales Rep')}
                  value={newName}
              ...
              <GlassField label={t('botPersona.descriptionOptionalLabel', 'Description (optional)')} className="persona-create-field" style={{ maxWidth: '100%' }}>
                <input
                  type="text"
                  placeholder={t('botPersona.descriptionPlaceholder', 'Short description of this persona')}
              ...
              <GlassField label={t('botPersona.systemPrompt', 'System Prompt')} className="persona-create-field" style={{ maxWidth: '100%' }}>
                <textarea
                  placeholder={t('botPersona.systemPromptPlaceholder', 'Write the system prompt that defines this persona\'s behavior, tone, and style...')}
            ...
            <div className="persona-preview-actions">
              ...
              >
                {creatingSaving ? t('botPersona.creating', 'Creating...') : (
                  <><Plus size={16} /> {t('botPersona.createPersona', 'Create Persona')}</>
                )}
              </UiButton>
            </div>
          </div>
