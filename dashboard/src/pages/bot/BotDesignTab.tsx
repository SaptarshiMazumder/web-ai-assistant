...
function defaultDesignState(): DesignState {
  return {
...
function widgetConfigToState(config: Record<string, unknown> | null): DesignState {
...
function stateToWidgetConfig(s: DesignState): Record<string, unknown> {
  return {
...
export default function BotDesignTab() {
  ...
  const update = useCallback(<K extends keyof DesignState>(key: K, value: DesignState[K]) => {
  ...
  const resetTextColor = () => update('textColor', '#ffffff')
