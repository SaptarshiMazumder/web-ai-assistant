...
class MenuExtractionRunner:
    def run(self, context: Dict[str, Any]) -> JobResult:
        ...
        if not bot_id:
            return JobResult(status="error", error="Missing bot_id for menu_extraction runner")
        if not gcs_prefix:
        ...
        # Import lazily to avoid module import cycles with crawl tasks.
        ...
        return JobResult(
            status="done",
            linked_job_type="menu_extraction",
